...
W_PSW=0.0664   # PSWSTR-2026-08-23. THE ONE UNFITTED WEIGHT IN _SIG -- set by blast radius, not by evidence.
...
_SIG=[('_zars',0.0088),('_zhh',0.2086),('_zla',0.1313),('_zdmg',0.0309),('_zpsw',0.0664),('_zhrc',0.4489),('_zpt',0.1051)]   # ISOREAL-2026-09-13: same seven terms as CAREERHR, refitted on a training table that now carries ACTUAL ISO. The CAREERHR fit measured _zdmg through a proxy (b_brl/b_xwobacon) because the Savant table had expected damage but no actual damage, and that caveat is now discharged: build_savant_training.py computes ISO = (2B + 2*3B + 3*HR)/AB straight off the Statcast event column, so _zdmg is once again actual/expected as DMGRATIO-2026-08-23 intended. Rebuild run 34763553648 (2h36m, 18.2MB); refit lab15 run 34772500739, which logs '_zdmg numerator: b_iso (REAL)' -- if that line ever says PROXY again the table regressed and these weights are void. 315,937 usable rows, 11.80% HR, 1,839 slates, grouped-CV by slate. WHAT CHANGED: _zdmg 0.0108 -> 0.0309, ~2.9x. The proxy was not neutral, it was quietly killing the term. Basket AUC 0.5893 -> 0.5939 alone and 0.6222 -> 0.6225 with the two career terms, so the headline barely moves -- _zhrc still carries the board -- but the damage term is now honest and the other five shift <0.004 each. All seven sum to 1.0000 (fitted 0.9999; the rounding rides on _zhrc) and all fitted POSITIVE. REVERT to the proxy fit: _SIG=[('_zars',0.0085),('_zhh',0.2121),('_zla',0.1362),('_zdmg',0.0108),('_zpsw',0.0674),('_zhrc',0.4598),('_zpt',0.1052)] with W_PSW=0.0674. REVERT to pre-CAREERHR: _SIG=[('_zars',0.0093),('_zhh',0.3655),('_zla',0.2834),('_zdmg',0.2818),('_zpsw',W_PSW)] and drop the _zhrc/_zpt rows + fetch_career + MIN_CAREER_G.
def _ms(key):
